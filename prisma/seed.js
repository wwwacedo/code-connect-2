...
async function main() {
	...
	// upsert method is used to create a new user if it doesn't exist
	const ana = await prisma.user.upsert({
		where: { username: author.username }, // username is unique
		update: {}, // empty update mean do nothing if user already exists
		create: author,
	})
	...
	posts.forEach(async (post) => {
		await prisma.post.upsert({
			where: { slug: post.slug }, // slug is unique
			update: {},
			create: post,
		})
	});
